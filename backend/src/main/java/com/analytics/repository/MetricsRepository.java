...
public interface MetricsRepository extends JpaRepository<Metric, Long> {
    ...
    @Query("""
        ...
        WHERE m.metricKey = :key
          AND m.recordedAt BETWEEN :from AND :to
        ORDER BY m.recordedAt ASC
        """)
    List<Metric> findByKeyAndPeriodOrdered(
        @Param("key") String key,
        @Param("from") LocalDateTime from,
        @Param("to") LocalDateTime to);

    @Query("SELECT m FROM Metric m WHERE m.metricKey = :key ORDER BY m.recordedAt DESC LIMIT 1")
    Metric findLatestByKey(@Param("key") String key);
}
